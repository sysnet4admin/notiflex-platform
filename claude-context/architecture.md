# Notiflex Platform Architecture (ch9 완료 시점)
...
| **worker-pool** | n2-standard-4       | **Kafka** 메시지 처리, 배치 작업 등             | 백그라운드 작업, Memory/IO 중심          |
| **ops-pool**  | e2-medium           | ArgoCD, Prometheus, Grafana, Loki, **Tempo, CronJobs** 등 운영 도구 | 안정적인 운영을 위한 격리된 환경         |

## 3. 컴포넌트 다이어그램 (Component Diagram)

'App of Apps' 패턴과 멀티테넌시, 비동기 메시징 및 분산 트레이싱이 적용된 컴포넌트 다이어그램입니다.
...
| | - with OTel SDK           |   | - Build, Push Image  |    |   - notiflex-smb.yaml                                                 |  |
| +---------------------------+   +----------------------+    |   - notiflex-enterprise.yaml                                          |  |
| +---------------------------+                               |   - notiflex-monitoring.yaml, notiflex-messaging.yaml                 |  |
| |   k8s Manifests (/k8s/)   |                               +-----------------------------------------------------------------------+  |
| | - /smb, /enterprise, /kafka |                                                                                                          |
| +---------------------------+                                                                                                            |
+--------------------------------------------------------------------|---------------------------------------------------------------------+
                                                                     |
                                                                     v
...
| +------------------------------------------------------------+     | +-------------------------------------------------------------------+ |
| | ArgoCD                                                     |     | | App of Apps                                                       | |
| | - Syncs and manages applications based on git manifests    |     | | - notiflex-smb, notiflex-enterprise, notiflex-monitoring, etc.    | |
| +------------------------------------------------------------+     | +-------------------------------------------------------------------+ |
|                                                                    |                                                                     |
|   +--------------------------------------------------------+       |       +-----------------------------------------------------------+   |
|   | Namespace: notiflex-smb (on api-pool)                  |------>|       | Namespace: kafka (on worker-pool)                         |   |
|   | +-----------------+    +----------------------------+  |       |       | +-------------------------------------------------------+ |   |
|   | | Notiflex SMB    |    | Valkey                     |  |       |       | | Kafka Cluster (Strimzi)                               | |   |
|   | | (OTel enabled)  |--> |                            |  |       |       | | - Pub/Sub for notifications                           | |   |
|   | +-----------------+    +----------------------------+  |       |       | +-------------------------------------------------------+ |   |
|   +--------------------------------------------------------+       |       +-----------------------------------------------------------+   |
|   +--------------------------------------------------------+       |          ^                                                            |
...
|   +--------------------------------------------------------+                  |                                                            |
|   +---------------------------------------------------------------------------|--------------------------------------------------------+   |
|   | Namespace: monitoring (on ops-pool)                                       | (traces)                                               |   |
|   | + Prometheus, Grafana, Loki, Alertmanager                                 |                                                        |   |
|   | +-------------------------------------------------------------------------+                                                        |   |
|   | | Tempo                                                                   | <-------------------------------------------------------+   |
|   | | - Receives traces from OTel enabled apps                                |                                                           |   |
|   | +-------------------------------------------------------------------------+                                                           |   |
|   +--------------------------------------------------------------------------------------------------------------------------------------+   |
|   | Namespace: default (on ops-pool)                                                                                                       |   |
|   | + CronJob (Healthcheck)                                                                                                                |   |
|   +----------------------------------------------------------------------------------------------------------------------------------------+   |
+------------------------------------------------------------------------------------------------------------------------------------------+
...
1.  **Commit & Push:** 개발자가 `notiflex-platform` 저장소의 `main` 브랜치에 코드를 커밋하고 푸시합니다. OpenTelemetry SDK를 사용하여 트레이스 생성 로직이 코드에 포함됩니다.
...
3.  **CD (ArgoCD):**
    ...
    -   이 디렉터리에 정의된 `Application` 리소스(e.g., `notiflex-smb`, `notiflex-messaging`)가 변경되면, ArgoCD는 해당 자식 애플리케이션들을 자동으로 동기화합니다.
    -   **Argo Rollouts**를 사용하여 점진적 배포(Canary)를 수행하여 안정성을 확보합니다.
4.  **Deployment:** ArgoCD에 의해 각 애플리케이션이 지정된 네임스페이스(`notiflex-smb`, `kafka`, `monitoring` 등)와 노드풀에 배포됩니다. Notiflex 앱에서 발생하는 트레이스는 Tempo로 전송됩니다.

## 5. 관측 가능성 (Observability) - MLT Stack

`monitoring` 네임스페이스와 `ops-pool` 노드풀을 중심으로 **메트릭(Metrics), 로그(Logs), 트레이스(Traces)**를 통합한 MLT 스택이 구축되어 있습니다.

| Tool         | Role                  | Description                                                                    |
| :----------- | :-------------------- | :----------------------------------------------------------------------------- |
| **Prometheus** | Metrics Monitoring    | 클러스터와 애플리케이션의 시계열 메트릭을 수집, 저장, 쿼리합니다.                      |
| **Grafana**    | Visualization & Dashboards | Prometheus, Loki, **Tempo**의 데이터를 통합 시각화하여 MLT 대시보드를 제공합니다. |
| **Loki**       | Log Aggregation       | 클러스터의 모든 파드에서 생성되는 로그를 수집하고 저장합니다.                           |
| **Tempo**      | **Distributed Tracing** | **OpenTelemetry** 기반 분산 트레이스를 수집, 저장, 쿼리하여 서비스 간 호출을 추적합니다. |
...
| `monitoring`           | Prometheus, Grafana, Loki, **Tempo** 등 관측 가능성 스택이 배포된 공간입니다.        |
| `kafka`                | **Strimzi Operator**와 **Kafka 클러스터**가 배포된 공간입니다. (ADR-013)        |
